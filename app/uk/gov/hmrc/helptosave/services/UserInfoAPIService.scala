...
class UserInfoAPIService @Inject()(userInfoAPIConnector: UserInfoAPIConnector,
  ...
  def getUserInfo(authorisationCode: String, nino: NINO)(implicit hc: HeaderCarrier, ec: ExecutionContext): Result[OpenIDConnectUserInfo] = for{
  ...
  /**
    ...
    */
  private def getUserInfoWithRetry(tokens: OAuthTokens, nino: NINO)(implicit hc: HeaderCarrier, ec: ExecutionContext): Result[OpenIDConnectUserInfo] = {
    val result: Future[Either[String, OpenIDConnectUserInfo]] = userInfoAPIConnector.getUserInfo(tokens).fold(
      {
  ...
  private def refreshTokensAndRetry(tokens: OAuthTokens)(implicit hc: HeaderCarrier, ec: ExecutionContext): Result[OpenIDConnectUserInfo] =
    for {
