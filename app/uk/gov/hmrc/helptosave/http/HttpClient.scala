...
package uk.gov.hmrc.helptosave.http
...
object HttpClient {
  ...
  implicit class HttpClientOps(val http: HttpClient) extends AnyVal {
    def get(
      ...
      http.GET(url, queryParams.toSeq, headers.toSeq)(rawHttpReads, hc, ec)
    ...
    def put[A](url: String, body: A, headers: Map[String, String] = Map.empty[String, String])(
      implicit w: Writes[A],
      hc: HeaderCarrier,
      ec: ExecutionContext): Future[HttpResponse] =
      http.PUT(url, body, headers.toSeq)(w, rawHttpReads, hc, ec)
  }
