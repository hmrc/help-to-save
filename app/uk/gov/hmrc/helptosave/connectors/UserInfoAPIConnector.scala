...
trait UserInfoAPIConnector {
  ...
  def getUserInfo(input: OAuthTokens)(implicit hc: HeaderCarrier, ec: ExecutionContext): EitherT[Future,APIError,UserInfo]
...
class UserInfoAPIConnectorImpl @Inject()(configuration: Configuration, ec: ExecutionContext) extends UserInfoAPIConnector {
  ...
  def getUserInfo(input: OAuthTokens)(implicit hc: HeaderCarrier, ec: ExecutionContext): EitherT[Future,APIError,UserInfo] = {
    ...
    EitherT[Future,APIError,UserInfo](http.get(url, headers)(hc, ec).map{ response ⇒
  ...
  private def handleOKResponse(response: HttpResponse): Either[APIError,UserInfo] =
    Try(response.json).map(_.validate[UserInfo]) match {
  ...
  private def handleUnauthorisedResponse(response: HttpResponse): Either[APIError,UserInfo] = {
  ...
  private def failure(message: String): Either[APIError,UserInfo] = Left(UnknownError(message))
