...
object PayePersonalDetails {
  ...
  def readSeq(json: JsValue, field: String, sequence: String): Option[JsValue] =
    (json \ field \ sequence).asOpt[JsValue]

  def read(json: JsValue, field: String): Option[JsValue] = (json \ field).asOpt[JsValue]

  def readName(json: JsValue): JsResult[Name] =
    readSeq(json, "names", "1")
      .orElse(readSeq(json, "names", "2"))
      .fold[JsResult[Name]](
  ...
  def readAddress(json: JsValue): JsResult[Address] =
    readSeq(json, "addresses", "2")
      .orElse(readSeq(json, "addresses", "1")) //1–Residential Address, 2–Correspondence Address
  ...
  def readPhoneNumber(json: JsValue): JsResult[Option[String]] =
    readSeq(json, "phoneNumbers", "7")
      .orElse(readSeq(json, "phoneNumbers", "1")) //7–Mobile Telephone Number, 1–Daytime Home Telephone Number
