...
class EmailStoreController @Inject() (emailStore: EmailStore, htsAuthConnector: AuthConnector)(
  ...
  def store(email: String): Action[AnyContent] = authorisedWithNino { implicit request ⇒ implicit nino ⇒
  ...
  def get(): Action[AnyContent] = authorisedWithNino { implicit request ⇒ implicit nino ⇒
