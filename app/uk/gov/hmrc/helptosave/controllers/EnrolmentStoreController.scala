...
class EnrolmentStoreController @Inject() (val enrolmentStore: EnrolmentStore,
  ...
  def setITMPFlag(): Action[AnyContent] = authorisedWithNino { implicit request ⇒ implicit nino ⇒
    handle(setITMPFlagAndUpdateMongo(nino), "set ITMP flag", nino)
  }

  def getEnrolmentStatus(): Action[AnyContent] = authorisedWithNino { implicit request ⇒ implicit nino ⇒
