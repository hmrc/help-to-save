...
class UCThresholdManagerSpec extends ActorTestSupport("UCThresholdManagerSpec") with Eventually {
  ...
  class TestApparatus {
    ...
    /** In tests we are typically getting the actor into the ready state by having the connectorProxy reply back to a
      * request. If we had something like this: ``
      ...
      *
      * // should be in ready state now actor ! UCThresholdManager.GetThresholdValue
      * expectMsg(UCThresholdManager.GetThresholdValueResponse(Some(1.0))) `` Then the danger is that the final
      * expectMsg fails. This is because when the connectorProxy replies to the threshold request, it is replying to a
      * temporary actor, not the UCThresholdManager actor itself. Thus, it is possible that the connectorProxy's
      * response doesn't reach the UCThresholdActor before the `UCThresholdManager.GetThresholdValue` message above.
      * Thus, the actor receives the latter message in the wrong state. To ensure that the
      * `UCThresholdManager.GetThresholdValue` reaches the actor in the ready state, the above should be re-written as:
      * `` connectorProxy.expectMsg(UCThresholdConnectorProxyActor.GetThresholdValue)
      * connectorProxy.reply(UCThresholdConnectorProxyActor.GetThresholdValueResponse(Right(1.0)))
      *
      * // should be in ready state now awaitInReadyState() actor ! UCThresholdManager.GetThresholdValue
    ...
    "handling requests to get the threshold value" must {
      ...
      "return the threshold value when the first call to DES on startup is successful but an error is returned from a second call " +
        "triggered by an eligibility check" in new TestApparatus {
          ...
          connectorProxy.send(sender1, UCThresholdConnectorProxyActor.GetThresholdValueResponse(Right(100.0)))
          connectorProxy
            .send(sender2, UCThresholdConnectorProxyActor.GetThresholdValueResponse(Left("No threshold found in DES")))
      ...
      "not schedule retries when a request has successfully returned the threshold from DES" in new TestApparatus {
        ...
        connectorProxy
          .send(sender1, UCThresholdConnectorProxyActor.GetThresholdValueResponse(Left("No threshold found in DES")))
